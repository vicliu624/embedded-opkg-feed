...
这是一个**面向嵌入式 Linux 设备的公开应用仓库**。它的目标很简单：让设备能通过 `opkg` 安装经过测试、与设备 ABI 严格匹配的应用包。
...
| 你的目标 | 从这里开始 |
| --- | --- |
| 我想在设备上安装应用 | [设备使用说明](docs/USAGE.zh-CN.md) |
| 我想提交自己的应用 | [贡献说明](CONTRIBUTING.zh-CN.md) |
...
1. **开发者提交应用源码和打包描述；维护者负责构建、测试、签名和发布。**
2. **GitHub Pages 是公开下载地址；签名索引用于证明下载内容确实由仓库维护者发布。**
3. **应用包不能替换系统核心部分。** libc、内核、系统服务、网络栈、`opkg` 本身和 KPU 驱动都属于固件责任范围，不从这里升级。
...
**首次使用不能跳过设备端准备。** 目标设备当前的 `opkg` 配置不完整，也没有可用的索引验签程序。因此必须先按照 [设备引导说明](docs/DEVICE_BOOTSTRAP.zh-CN.md) 重新构建并刷入对应的基础固件。完成后，设备才能安全地信任公开仓库。
...
真实仓库地址、密钥安装方式和故障排查在 [设备使用说明](docs/USAGE.zh-CN.md) 中。`tdvp-hello` 只是验证整条流程的示例，不是必须安装的功能应用。
...
1. Fork 本仓库并新建分支。
2. 复制 `packages/_template/`，把目录和 `package.env` 改成你的应用信息。
3. 把应用运行需要的文件放进 `root/`；文件在安装后会原样出现在设备根目录下。
4. 在你的开发环境中运行构建和检查命令。
...
- `tdvp-` 前缀保留给平台维护者发布的官方包，例如 `tdvp-hello`。
- 社区包建议使用 `<github 用户名>-<应用名>`，例如 `alice-status-panel`，以避免重名。
- 包必须声明支持的平台。构建脚本会自动加入该平台的 ABI 依赖，不能靠手工省略。
- 不允许应用包写入或替换 `/boot`、`/lib`、`/lib64`、`/usr/lib`、`/usr/lib/systemd`、`/usr/sbin`。这能避免公开应用包意外覆盖系统底座。
