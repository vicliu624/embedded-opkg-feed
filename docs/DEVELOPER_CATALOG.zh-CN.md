...
| 源码维护 | `patch`、`diff`、`tdvp-grep`、`find`、`sed`、`awk`、`less`、`file`、`tree`、`jq`、`curl`、`wget` | 不覆盖 BusyBox；安全的标准名可由 `/usr/bin` 前端指向包私有实现，固件已有命令使用明确的 `tdvp-*` 名称 |
...
的入口为 `/usr/bin/tdvp-grep`。固件的 `/usr/bin/awk` 和 `/usr/bin/grep` 都保持不变。
