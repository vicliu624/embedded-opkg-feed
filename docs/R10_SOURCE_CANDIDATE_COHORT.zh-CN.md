...
| 37 | `psmisc`（source lock/静态门禁已就绪；待 GitHub Actions） | 23.7 | `process-inspection-tools` 是第一项明确的上游版本增量：固定 Buildroot 2025.02.1 的 `psmisc.mk` 仅作为受审交叉构建逻辑，单一受限 Make 参数把其 `PSMISC_VERSION` 从基线 23.5 覆盖为官方当前 release 23.7；`psmisc-23.7.tar.xz` 的流式 SHA-256 为 `58c55d9c1402474065adae669511c191de374b0871eec781239ab400b907c327`。因基线 `psmisc.hash` 不含新 release，构建事务只能临时以 `source.lock` 精确同名同哈希的单行 override 供正常 Buildroot SHA-256 检查，且无论成功或失败均按字节还原原始清单；它不关闭或绕过哈希验证。payload 只允许私有 `fuser`、`killall`、`pslog`、`prtstat`、`pstree` ELF 和对应 `tdvp-*` frontend；`peekfd` 因 23.7 尚无 RISC-V 实现而明确排除。它只精确复用 immutable `libncursesw (= 6.4-20230603-1)`，不得发布 shared ABI、headers、manpages、locale 或普通 firmware command path。CI 只允许构建/ELF/closure 审计，绝不运行任一命令、读取 procfs、传入 PID/名称/路径/socket、发送信号或遍历真实进程树。Actions source batch 成功、无重编 merge、设备生命周期验证、签名与发布均仍未发生。 |
