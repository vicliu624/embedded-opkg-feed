...
| 34 | `pigz`（来源锁与静态策略已审查；待 GitHub Actions source batch） | 2.8 | `parallel-gzip-tools` 只允许私有 `pigz` ELF 与 `tdvp-pigz`。官方 `zlib.net` 当前发布的 `pigz-2.8.tar.gz` 已以 SHA-256 `eb872b4f0e1f0ebe59c9f7bd8c506c4204893ba6a8492de31df416f0d5170fd0` 流式复算，并与 Buildroot 2025.02.1 的锁定版本一致。它只消费 immutable target catalogue 的 `libz (= 1.3.1-1)`，不发布 zlib 或其他 shared-runtime ABI，也不占用 BusyBox `gzip` 路径。CI 只可构建和审计 ELF，绝不运行 `pigz`、传入内容、压缩或解压数据；压缩与 CRC 不得被当作内容认证。source batch 成功后才可记录 IPK 证据，随后仍需无重编 merge 与实机生命周期验证。 |
...
`built *.ipk` 记录逐一去重后，当前 112 个 r10 recipe 中已有 95 个 recipe package 具备实际 source-build
证据（第 95 个是 `xxhash` 的 run `34000078942`，并已由 36-artifact 无重编 merge `34000394791` 纳入 merged candidate）；其余 12 个通用 package 是 immutable target catalogue provider：`ca-certificates`、`libatomic-1`、
...
Cardputer 应用不属于本 release。新加入的 `pigz` 仅完成来源锁和静态策略准入，尚未拥有 GitHub Actions
source-build 证据，因而不计入 95。
