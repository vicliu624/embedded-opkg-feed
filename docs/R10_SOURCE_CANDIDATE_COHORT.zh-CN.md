...
| 34 | `pigz`（GitHub Actions source batch 已通过；待无重编 merge/实机） | 2.8 | `parallel-gzip-tools` 只允许私有 `pigz` ELF 与 `tdvp-pigz`。官方 release/Buildroot 锁定 SHA-256 是 `eb872b4f0e1f0ebe59c9f7bd8c506c4204893ba6a8492de31df416f0d5170fd0`；原 `zlib.net` 端返回不匹配内容后，仅因 OmniOS HTTPS mirror 的 121,304-byte archive 逐字节匹配该 hash 才切换传输端。run [`34001592645`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001592645) 恢复既有 SDK 而未重建，实际以 `riscv64-unknown-linux-gnu-gcc` 构建/链接 `pigz`，生成 `pigz_2.8-1_riscv64.ipk`，通过 source cache、RISC-V ELF、runtime closure、deny overlay、feed verification 和 445-object target coverage；SDK-base 和 merge job 均 skipped。source artifact [`9979676644`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001592645/artifacts/9979676644) 为 90,155,403 bytes，GitHub SHA-256 `a022179609394e636be83251319b31af957d52fd47366fcf78bc15058c3791c4`。它只消费 immutable `libz (= 1.3.1-1)`，不发布 zlib/其他 shared-runtime ABI，也不占用 BusyBox `gzip` 路径；CI 未运行、传入内容、压缩或解压数据。压缩与 CRC 绝非内容认证。仍须无重编 merge 和实机生命周期验证。 |
...
**r10 实际 package inventory（2026-09-06）。** 对 36 个成功 source batch 的 GitHub Actions
`built *.ipk` 记录逐一去重后，当前 112 个 r10 recipe 中已有 96 个 recipe package 具备实际 source-build
证据（第 96 个是 `pigz` 的 run `34001592645`；它尚待无重编 merge。此前第 95 个 `xxhash` 已由 36-artifact 无重编 merge `34000394791` 纳入 merged candidate）；其余 12 个通用 package 是 immutable target catalogue provider：`ca-certificates`、`libatomic-1`、
...
Cardputer 应用不属于本 release。`pigz` 已计入 96 个 source-build package，但仍未签名、发布、部署或在设备执行；先由后续无重编 merge 验证其 exact artifact/IPK hash。
