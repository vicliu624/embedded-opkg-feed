...
| 38 | `net-tools`（source lock/静态门禁已就绪；待 GitHub Actions） | 2.10 | `legacy-network-tools` 只采用与固定 Buildroot 2025.02.1 完全一致的 `net-tools-2.10.tar.xz`（SHA-256 `b262435a5241e89bfa51c3cabd5133753952f7a7b7b93f32e08cb9d96f580d69`）和受审交叉构建逻辑。候选 payload 只允许私有 `arp`、`ifconfig`、`ipmaddr`、`iptunnel`、`mii-tool`、`nameif`、`netstat`、`plipconfig`、`rarp`、`route`、`slattach` ELF 及对应 `tdvp-*` frontend；不覆盖 BusyBox path，不安装配置、服务、headers、static archive、pkg-config metadata 或 shared-runtime ABI。CI 只允许构建/ELF/closure 审计，绝不执行命令、读取接口状态、传入 interface/address/route/device，建立连接或修改网络配置。Actions source batch、无重编 merge、设备侧网络生命周期验证、签名和发布均未发生。 |
...
`built *.ipk` 记录逐一去重后，当前 116 个 r10 recipe 中已有 99 个 recipe package 具备实际 source-build
及无重编 merge 证据（第 99 个 `psmisc` 已由 40-artifact 无重编 merge [`34006620790`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006620790) 纳入 merged candidate；第 98 个 `lzop` 已由 39-artifact 无重编 merge [`34004352420`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004352420) 纳入；第 97 个 `lz4` 已由 38-artifact 无重编 merge `34003078163` 纳入；第 96 个 `pigz` 已由 37-artifact 无重编 merge `34001947527` 纳入；此前第 95 个 `xxhash` 已由 36-artifact 无重编 merge `34000394791` 纳入）。另有 1 个 `net-tools` 已完成 source lock/静态门禁而待 Actions；其余 12 个通用 package 是 immutable target catalogue provider：`ca-certificates`、`libatomic-1`、
