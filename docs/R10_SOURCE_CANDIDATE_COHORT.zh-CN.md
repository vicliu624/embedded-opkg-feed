...
| 36 | `lzop`（已锁定；待 GitHub Actions source batch） | 1.04（LZO 2.10） | `parallel-lzo-tools` 将只允许私有 `lzop` ELF 与 `tdvp-lzop` frontend。官方 HTTPS archive 的 LZO SHA-256 `c0f892943208266f9b6543b3ae308fab6284c5c90e627931446fb49b4221a072`、lzop SHA-256 `7e72b62a8a60aff5200a047eea0773a8fb205caf7acbe1774d95147f305a2f41` 已流式复算，并与 Buildroot 2025.02.1 的 `LZO_VERSION = 2.10`、`LZOP_VERSION = 1.04` 一致。recipe 强制 `LZO_CONF_OPTS=-DENABLE_SHARED=OFF -DENABLE_STATIC=ON`，因此 LZO 只能是 transaction 内的 static implementation；generic helper 只能复制 `lzop` 到 `/usr/libexec/tdvp-lzop`，禁止 `liblzo2.so`、`liblzo2.a`、headers、pkg-config metadata 或普通 `/usr/bin/lzop` 进入 IPK。只有 GitHub Actions 的 locked-cache、RISC-V ELF、runtime closure、deny overlay、feed verification 与 target coverage 全部通过后，才可作为 source artifact；若命令仍携带 `liblzo2` 动态依赖，closure gate 必须 fail-closed。CI 不得运行 lzop、传入内容、压缩或解压数据；frame/checksum 不得被当作内容认证。 |
...
**r10 实际 package inventory（2026-09-06）。** 对 38 个成功 source batch 的 GitHub Actions
`built *.ipk` 记录逐一去重后，当前 114 个 r10 recipe 中已有 97 个 recipe package 具备实际 source-build
及无重编 merge 证据（第 97 个 `lz4` 已由 38-artifact 无重编 merge [`34003078163`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34003078163) 纳入 merged candidate；第 96 个 `pigz` 已由 37-artifact 无重编 merge `34001947527` 纳入；此前第 95 个 `xxhash` 已由 36-artifact 无重编 merge `34000394791` 纳入；第 98 个候选 `lzop` 已锁定但尚无 source-build 证据）；其余 12 个通用 package 是 immutable target catalogue provider：`ca-certificates`、`libatomic-1`、
