...
| 33 | `xxhash`（GitHub Actions source batch 已通过，待无重编 merge/实机） | 0.8.3 | `fast-integrity-tools` 只允许私有 `xxhsum` ELF 与 `tdvp-xxhsum`。固定 Buildroot 2025.02.1 的 `xxhash-0.8.3.tar.gz` SHA-256 `aae608dfe8213dfd05d909a57718ef82f30722c392344583d3f39050c7f29a80` 已由官方 GitHub tag archive 流式复算；recipe 仅传入 `XXHASH_TARGETS=xxhsum` 和 `XXHASH_INSTALL_TARGETS=install_xxhsum`，实际不构建/安装 `libxxhash.so`、静态库、headers 或 pkg-config metadata。run [`34000078942`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34000078942) 在已恢复、未重建的 K230 SDK 上实际以 `riscv64-unknown-linux-gnu-gcc` 构建/链接 `xxhsum`，生成 `xxhash_0.8.3-1_riscv64.ipk`，并通过 source cache、RISC-V ELF、runtime closure、deny overlay、feed verification 和 445 个 non-ABI dynamic object 的 target-runtime coverage；SDK-base 和 merge job 都是 skipped。source artifact [`9979257288`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34000078942/artifacts/9979257288) 为 90,122,393 bytes，GitHub SHA-256 `614f484c613a024b831ff62f3e2f440259f5260788ad28dd01756b977632c2f6`。`xxhsum` 是非加密快速校验工具，只能用于受控去重或非安全预筛选，绝不可替代认证、签名或安全完整性校验；仍须后续无重编 merge 与实机生命周期验证。 |
...
证据（第 95 个是 `xxhash` 的 run `34000078942`）；其余 12 个通用 package 是 immutable target catalogue provider：`ca-certificates`、`libatomic-1`、
