...
| 35 | `lz4`（来源锁与静态策略已审查；待 GitHub Actions source batch） | 1.10.0 | `fast-lz4-tools` 只允许私有 `lz4`、`lz4c`、`unlz4`、`lz4cat` ELF 与同名 `tdvp-` frontend。官方 tag archive SHA-256 `537512904744b35e232912055ccf8ec66d768639ff3abe5788d90d792ec5f48b` 已流式复算，并与 Buildroot 2025.02.1 的 `LZ4_VERSION = 1.10.0` 一致。transaction 将 `LZ4_MAKE_OPTS=BUILD_SHARED=no`，仅允许 build-time private static implementation；generic helper 只复制四个 command ELF 到 `/usr/libexec/tdvp-lz4`，禁止 `liblz4.so`、`liblz4.a`、headers、pkg-config metadata 与普通 `/usr/bin/lz4` 路径进入 IPK。CI 只能构建/审计 ELF，绝不运行 LZ4、传入内容、压缩或解压数据；frame/checksum 不能被当作内容认证。source batch 成功后才可记录 IPK 证据，随后仍需无重编 merge 与实机生命周期验证。 |
...
`built *.ipk` 记录逐一去重后，当前 113 个 r10 recipe 中已有 96 个 recipe package 具备实际 source-build
证据（第 96 个 `pigz` 的 run `34001592645` 已由 37-artifact 无重编 merge `34001947527` 纳入 merged candidate；此前第 95 个 `xxhash` 已由 36-artifact 无重编 merge `34000394791` 纳入；新 `lz4` 尚无 Actions source-build 证据，故不计入）；其余 12 个通用 package 是 immutable target catalogue provider：`ca-certificates`、`libatomic-1`、
