...
**r10 实际 package inventory（2026-09-06）。** 对 37 个成功 source batch 的 GitHub Actions
`built *.ipk` 记录逐一去重后，当前 113 个 r10 recipe 中已有 97 个 recipe package 具备实际 source-build
证据（第 97 个是 `lz4` 的 run `34002789451`，尚待无重编 merge；第 96 个 `pigz` 已由 37-artifact 无重编 merge `34001947527` 纳入 merged candidate；此前第 95 个 `xxhash` 已由 36-artifact 无重编 merge `34000394791` 纳入）；其余 12 个通用 package 是 immutable target catalogue provider：`ca-certificates`、`libatomic-1`、
