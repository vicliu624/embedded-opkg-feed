...
| 33 | `xxhash`（已完成 source-batch 前静态准入；尚未构建） | 0.8.3 | `fast-integrity-tools` 计划只允许私有 `xxhsum` ELF 与 `tdvp-xxhsum`。固定 Buildroot 2025.02.1 的 `xxhash-0.8.3.tar.gz` SHA-256 `aae608dfe8213dfd05d909a57718ef82f30722c392344583d3f39050c7f29a80` 已由官方 GitHub tag archive 流式复算；recipe 仅传入 `XXHASH_TARGETS=xxhsum` 和 `XXHASH_INSTALL_TARGETS=install_xxhsum`，因此不构建/安装 `libxxhash.so`、静态库、headers 或 pkg-config metadata。`xxhsum` 是非加密快速校验工具，只能用于受控的去重或非安全预筛选，绝不可替代认证、签名或安全完整性校验。下一步仅允许 GitHub Actions 的单包 source build；本地不编译 K230/Buildroot，成功后才记录 artifact 并发起无重编 merge。 |
...
**r10 实际 package inventory（2026-09-06）。** 对 35 个成功 source batch 的 GitHub Actions
`built *.ipk` 记录逐一去重后，新增 `xxhash` 前的 110 个 r10 recipe 中已有 94 个 recipe package 具备实际 source-build
证据。当前工作树的第 111 个 recipe `xxhash` 已完成来源/静态准入，尚待它自己的单包 Actions source build；不得用该计划状态冒充已构建证据。其余 12 个通用 package 是 immutable target catalogue provider：`ca-certificates`、`libatomic-1`、
