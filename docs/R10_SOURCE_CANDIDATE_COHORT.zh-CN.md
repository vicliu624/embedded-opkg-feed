...
| 33 | `findutils`（GitHub Actions source batch 待执行） | 4.10.0 | `filesystem-search-tools` 只允许私有 `find`/`xargs` ELF 与 `tdvp-find`/`tdvp-xargs`；不复制 Buildroot 的 `locate`/`updatedb` 或任何共享 runtime provider。上游 GNU HTTPS archive 已于 2026-09-06 流式哈希验证为锁定 SHA-256，且 Debian security tracker 将已知 findutils 问题标为 resolved。待 GitHub Actions 在已恢复的 K230 SDK cache 中构建并审计 IPK；CI 不得执行 `find`、传入 filesystem path、执行 `xargs` 或启动子命令。 |
