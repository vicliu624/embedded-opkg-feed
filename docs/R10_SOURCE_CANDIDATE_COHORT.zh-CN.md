...
`built *.ipk` 记录逐一去重后，当前 115 个 r10 recipe 中已有 99 个 recipe package 具备实际 source-build
及无重编 merge 证据（第 99 个 `psmisc` 已由 40-artifact 无重编 merge [`34006620790`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006620790) 纳入 merged candidate；第 98 个 `lzop` 已由 39-artifact 无重编 merge [`34004352420`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004352420) 纳入；第 97 个 `lz4` 已由 38-artifact 无重编 merge `34003078163` 纳入；第 96 个 `pigz` 已由 37-artifact 无重编 merge `34001947527` 纳入；此前第 95 个 `xxhash` 已由 36-artifact 无重编 merge `34000394791` 纳入）；其余 12 个通用 package 是 immutable target catalogue provider：`ca-certificates`、`libatomic-1`、
