...
| 31 | `bwm-ng`（GitHub Actions source batch/无重编 merge 已通过，待实机） | 0.6.3 | `bandwidth-monitor-tools` 只允许私有 `bwm-ng` ELF 与 `tdvp-bwm-ng`。run [`33997001920`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33997001920) 用锁定来源实际构建一个 IPK，并通过 source cache、RISC-V ELF、runtime closure、deny overlay 与 feed verification；source artifact [`9978397329`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33997001920/artifacts/9978397329) 为 90,164,043 bytes，SHA-256 `affabae7d97bb5778227d09ae634d867a17f11f9b5a32d6d68f568a6b91fb373`。其可选 terminal view 只消费 immutable target `libncursesw`，不构建或发布 runtime provider。run [`33997364665`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33997364665) 只 hash-merge 34 个 artifact、重建索引并通过 runtime closure 和 445 个 non-ABI dynamic object 的 target-runtime coverage；merged artifact [`9978511004`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33997364665/artifacts/9978511004) 为 196,699,030 bytes，SHA-256 `ffeb22475b948bc57064df291f8783f5006128c2965cab9feddb73bb766472c0`。CI 未执行监视器、读取 procfs、观察网络接口或观察磁盘 I/O。 |
| 32 | `rhash`（GitHub Actions source batch/无重编 merge 已通过，待实机） | 1.4.4 | `checksum-tools` 只允许私有 `rhash` ELF 与 `tdvp-rhash`。run [`33998249379`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33998249379) 用锁定 RHash 来源实际构建一个 IPK，并通过 source cache、RISC-V ELF、runtime closure、deny overlay、feed verification 和 445 个 non-ABI dynamic object 的 target-runtime coverage；source artifact [`9978739567`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33998249379/artifacts/9978739567) 为 90,288,593 bytes，SHA-256 `85f7c5752bfadd140bd9a34c9c7cc7069890461e23f57b5f5e63e9b670ae65c3`。recipe 显式关闭 gettext/OpenSSL、静态链接 private `librhash` implementation 并关闭 shared library；不构建或发布 runtime provider。run [`33998522557`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33998522557) 只 hash-merge 35 个 artifact、重建索引，并在未编译的情况下通过 runtime closure 和 445 个 non-ABI dynamic object 的 target-runtime coverage；merged artifact [`9978839795`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33998522557/artifacts/9978839795) 为 196,900,178 bytes，SHA-256 `6042b30b4b5e9af1d46b50c8b0a18a8e20564e189919feb5734c103251bcb14e`。CI 未执行该命令、传入文件路径或读取 payload。 |
| 33 | `xxhash`（GitHub Actions source batch/无重编 merge 已通过，待实机） | 0.8.3 | `fast-integrity-tools` 只允许私有 `xxhsum` ELF 与 `tdvp-xxhsum`。固定 Buildroot 2025.02.1 的 `xxhash-0.8.3.tar.gz` SHA-256 `aae608dfe8213dfd05d909a57718ef82f30722c392344583d3f39050c7f29a80` 已由官方 GitHub tag archive 流式复算；recipe 仅传入 `XXHASH_TARGETS=xxhsum` 和 `XXHASH_INSTALL_TARGETS=install_xxhsum`，实际不构建/安装 `libxxhash.so`、静态库、headers 或 pkg-config metadata。run [`34000078942`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34000078942) 在已恢复、未重建的 K230 SDK 上实际以 `riscv64-unknown-linux-gnu-gcc` 构建/链接 `xxhsum`，生成 `xxhash_0.8.3-1_riscv64.ipk`，并通过 source cache、RISC-V ELF、runtime closure、deny overlay、feed verification 和 445 个 non-ABI dynamic object 的 target-runtime coverage；SDK-base 和 merge job 都是 skipped。source artifact [`9979257288`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34000078942/artifacts/9979257288) 为 90,122,393 bytes，GitHub SHA-256 `614f484c613a024b831ff62f3e2f440259f5260788ad28dd01756b977632c2f6`。run [`34000394791`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34000394791) 只下载并逐哈希比对 36 个 source artifact，重建索引且在未编译的情况下再次通过 runtime closure 与 445-object target-runtime coverage；SDK-base/package-build job 均 skipped。merged artifact [`9979351229`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34000394791/artifacts/9979351229) 为 196,935,108 bytes，GitHub SHA-256 `0c2ae26b0841902ca0e850ddb76306aaba64732e2ec8715b3240396ec5f19b7b`。`xxhsum` 是非加密快速校验工具，只能用于受控去重或非安全预筛选，绝不可替代认证、签名或安全完整性校验；仍须实机生命周期验证。 |
| 34 | `pigz`（GitHub Actions source batch/无重编 merge 已通过，待实机） | 2.8 | `parallel-gzip-tools` 只允许私有 `pigz` ELF 与 `tdvp-pigz`。官方 release/Buildroot 锁定 SHA-256 是 `eb872b4f0e1f0ebe59c9f7bd8c506c4204893ba6a8492de31df416f0d5170fd0`；原 `zlib.net` 端返回不匹配内容后，仅因 OmniOS HTTPS mirror 的 121,304-byte archive 逐字节匹配该 hash 才切换传输端。run [`34001592645`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001592645) 恢复既有 SDK 而未重建，实际以 `riscv64-unknown-linux-gnu-gcc` 构建/链接 `pigz`，生成 `pigz_2.8-1_riscv64.ipk`，通过 source cache、RISC-V ELF、runtime closure、deny overlay、feed verification 和 445-object target coverage；SDK-base 和 merge job 均 skipped。source artifact [`9979676644`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001592645/artifacts/9979676644) 为 90,155,403 bytes，GitHub SHA-256 `a022179609394e636be83251319b31af957d52fd47366fcf78bc15058c3791c4`。run [`34001947527`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001947527) 随后只下载、逐一核对 37 个 source artifact 的 manifest/IPK hash、重建索引，并在未编译的情况下通过 runtime closure 与 445-object target coverage；SDK-base 与 package-build job 都是 skipped。merged artifact [`9979808940`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001947527/artifacts/9979808940) 为 197,003,060 bytes，GitHub SHA-256 `ad8f399d2cbdb0ea1101ce7cb4c03ceb0063021018d968e4da8222f4153950e6`。它只消费 immutable `libz (= 1.3.1-1)`，不发布 zlib/其他 shared-runtime ABI，也不占用 BusyBox `gzip` 路径；CI 未运行、传入内容、压缩或解压数据。压缩与 CRC 绝非内容认证。artifact 仍未签名、发布、部署或在设备执行，仍须实机生命周期验证。 |
| 35 | `lz4`（GitHub Actions source batch/无重编 merge 已通过，待实机） | 1.10.0 | `fast-lz4-tools` 只允许私有 `lz4`、`lz4c`、`unlz4`、`lz4cat` ELF 与同名 `tdvp-` frontend。官方 tag archive SHA-256 `537512904744b35e232912055ccf8ec66d768639ff3abe5788d90d792ec5f48b` 已流式复算，并与 Buildroot 2025.02.1 的 `LZ4_VERSION = 1.10.0` 一致。run [`34002789451`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34002789451) 恢复既有 SDK 而未重建，以 `BUILD_SHARED=no` 构建 Buildroot 的 `lib/` 与 `programs/`，生成 `lz4_1.10.0-1_riscv64.ipk`，并通过 source cache、RISC-V ELF、runtime closure、deny overlay、feed verification 和 445-object target coverage；SDK-base 与 merge job 均 skipped。source artifact [`9980027622`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34002789451/artifacts/9980027622) 为 90,226,771 bytes，GitHub SHA-256 `492cc56e4c238cba213ed082db6c0eca955155d12b709711104f9c171f67b3ba`。generic helper 只复制四个 command ELF 到 `/usr/libexec/tdvp-lz4`，没有 `liblz4.so`、`liblz4.a`、headers、pkg-config metadata 或普通 `/usr/bin/lz4` 路径进入 IPK。随后无重编 merge run [`34003078163`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34003078163) 下载并逐一比对全部 38 个 source artifact 的 manifest/IPK hash、重建索引且未运行编译，并再次通过 feed verification、runtime closure 和 445-object target coverage；SDK-base 与 package-build job 均 skipped。merged unsigned artifact [`9980152139`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34003078163/artifacts/9980152139) 为 197,142,393 bytes，GitHub SHA-256 `0964bb1eb75080b69951c3c00808a72398d2a75651162708e79c13d1a062bdaa`。CI 未运行 LZ4、传入内容、压缩或解压数据；frame/checksum 不能被当作内容认证。所有 artifact 均未签名、发布、部署或在设备执行，仍须实机生命周期验证。 |
| 36 | `lzop`（GitHub Actions source batch/无重编 merge 已通过，待实机） | 1.04（LZO 2.10） | `parallel-lzo-tools` 只允许私有 `lzop` ELF 与 `tdvp-lzop` frontend。官方 HTTPS archive 的 LZO SHA-256 `c0f892943208266f9b6543b3ae308fab6284c5c90e627931446fb49b4221a072`、lzop SHA-256 `7e72b62a8a60aff5200a047eea0773a8fb205caf7acbe1774d95147f305a2f41` 已流式复算，并与 Buildroot 2025.02.1 的 `LZO_VERSION = 2.10`、`LZOP_VERSION = 1.04` 一致。run [`34004037276`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004037276) 恢复既有 SDK 而未重建，实际以 `-DENABLE_SHARED=OFF -DENABLE_STATIC=ON` 配置 LZO；日志确认只链接 `liblzo2.a`，而非 shared object。随后实际交叉构建 `lzop_1.04-1_riscv64.ipk`，并通过 locked source、RISC-V ELF、runtime closure、deny overlay、feed verification 与 445-object target coverage；SDK-base 与 merge job 均 skipped。source artifact [`9980420311`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004037276/artifacts/9980420311) 为 90,140,564 bytes，GitHub SHA-256 `698a96674a57f637128102e0c8d15264de49f727e9862f919f36a5cc7ee53cad`。随后无重编 merge run [`34004352420`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004352420) 下载并逐一比对全部 39 个 source artifact 的 manifest/IPK hash、重建索引且未运行编译，并再次通过 feed verification、runtime closure 和 445-object target coverage；SDK-base 与 package-build job 均 skipped。merged unsigned artifact [`9980529319`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004352420/artifacts/9980529319) 为 197,195,504 bytes，GitHub SHA-256 `0847c8f87bb8c3d647a5aed9c27bf8f3e442521ba32e96cb81d404390b1f3201`。generic helper 只复制 `lzop` 到 `/usr/libexec/tdvp-lzop`，没有 `liblzo2.so`、`liblzo2.a`、headers、pkg-config metadata 或普通 `/usr/bin/lzop` 路径进入 IPK；若存在 `liblzo2` 动态依赖，closure gate 会 fail-closed。CI 未运行 lzop、传入内容、压缩或解压数据；frame/checksum 不得被当作内容认证。所有 artifact 均未签名、发布、部署或在设备执行，仍须实机生命周期验证。 |

| 37 | `psmisc`（GitHub Actions source batch/无重编 merge 已通过，待实机） | 23.7 | `process-inspection-tools` 是第一项明确的上游版本增量：固定 Buildroot 2025.02.1 的 `psmisc.mk` 仅作为受审交叉构建逻辑，单一受限 Make 参数把其 `PSMISC_VERSION` 从基线 23.5 覆盖为官方当前 release 23.7；`psmisc-23.7.tar.xz` 的流式 SHA-256 为 `58c55d9c1402474065adae669511c191de374b0871eec781239ab400b907c327`。因基线 `psmisc.hash` 不含新 release，构建事务只能临时以 `source.lock` 精确同名同哈希的单行 override 供正常 Buildroot SHA-256 检查，且无论成功或失败均按字节还原原始清单；它不关闭或绕过哈希验证。run [`34006256552`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552) 恢复既有 SDK 而未重建，实际从锁定 cache 构建 `psmisc_23.7-1_riscv64.ipk`，通过 source cache、RISC-V ELF、runtime closure、deny overlay、feed verification 与 445-object target coverage；SDK-base 与 merge job 均 skipped。source artifact [`9981057795`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552/artifacts/9981057795) 为 90,137,169 bytes，GitHub SHA-256 `db9fcdbde1a26de09dcb5270f53213d9c5d514c1a50521243e7442eb7525f58a`。随后无重编 merge run [`34006620790`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006620790) 下载并逐一核对全部 40 个 source artifact 的 manifest/IPK hash，重建 index 且未运行编译，并再次通过 feed verification、runtime closure 和 445-object target coverage；SDK-base 与 package-build job 均 skipped。merged unsigned artifact [`9981196020`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006620790/artifacts/9981196020) 为 197,245,213 bytes，GitHub SHA-256 `3abcbefac768ad21ebe49df0d54050b0d191e2891687e495dc0899a19d53e91d`。payload 只允许私有 `fuser`、`killall`、`pslog`、`prtstat`、`pstree` ELF 和对应 `tdvp-*` frontend；`peekfd` 因 23.7 尚无 RISC-V 实现而明确排除。它只精确复用 immutable `libncursesw (= 6.4-20230603-1)`，不得发布 shared ABI、headers、manpages、locale 或普通 firmware command path。CI 只允许构建/ELF/closure 审计，绝不运行任一命令、读取 procfs、传入 PID/名称/路径/socket、发送信号或遍历真实进程树。所有 artifact 仍未签名、发布、部署或在设备执行，仍须实机生命周期验证。 |
| 38 | `net-tools`（GitHub Actions source batch/无重编 merge 已通过，待实机） | 2.10 | `legacy-network-tools` 只采用与固定 Buildroot 2025.02.1 完全一致的 `net-tools-2.10.tar.xz`（SHA-256 `b262435a5241e89bfa51c3cabd5133753952f7a7b7b93f32e08cb9d96f580d69`）和受审交叉构建逻辑。run [`34007398511`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007398511) 恢复既有 SDK 而未重建，实际交叉构建 `net-tools_2.10-1_riscv64.ipk`，并通过 locked source、RISC-V ELF、runtime closure、deny overlay、feed verification 与 445-object target coverage；SDK-base 与 merge job 均 skipped。source artifact [`9981425511`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007398511/artifacts/9981425511) 为 90,328,415 bytes，GitHub SHA-256 `9761e314772218eed4b4ee9c6fd14ef50fadfd158289b3a443fc35c80c8efc25`。随后无重编 merge run [`34007657490`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007657490) 下载并逐一核对全部 41 个 source artifact 的 manifest/IPK hash、重建 index 且未运行编译，并再次通过 feed verification、runtime closure 和 445-object target coverage；SDK-base 与 package-build job 均 skipped。merged unsigned artifact [`9981521824`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007657490/artifacts/9981521824) 为 197,486,158 bytes，GitHub SHA-256 `061756699b615cc167773297cec41b03562588eba3bd296aeefa3202f4f52ba2`。payload 只允许私有 `arp`、`ifconfig`、`ipmaddr`、`iptunnel`、`mii-tool`、`nameif`、`netstat`、`plipconfig`、`rarp`、`route`、`slattach` ELF 及对应 `tdvp-*` frontend；不覆盖 BusyBox path，不安装配置、服务、headers、static archive、pkg-config metadata 或 shared-runtime ABI。CI 只允许构建/ELF/closure 审计，绝不执行命令、读取接口状态、传入 interface/address/route/device，建立连接或修改网络配置。所有 artifact 均未签名、发布、部署或在设备执行，仍须设备侧网络生命周期验证。 |
...
source，batch 入口和配方改动均已撤回。后续重复的 `filesystem-search-tools` run
[`33999193609`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33999193609) 虽通过相同的
ELF、runtime closure、445-object target coverage 与 feed verification gate，并上传 artifact
[`9979010065`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33999193609/artifacts/9979010065)
（90,316,023 bytes；zip SHA-256 `18b424cd02e52b231da359ca35b166173c259b2ef86b56f5b268a33d1d22df23`），但未增加新的
IPK，故明确排除在 merge source 之外；其临时 batch 入口也已撤回。旧的成功 source batch 仍是这六个包唯一可复用的 r10 构建证据。
以后新增候选必须先比对成功 batch 的实际 IPK 清单，不能以“仓库中有配方”误判为尚未构建。

**r10 实际 package inventory（2026-09-06）。** 对 39 个成功 source batch 的 GitHub Actions
`built *.ipk` 记录逐一去重后，当前 116 个 r10 recipe 中已有 100 个 recipe package 具备实际 source-build
及无重编 merge 证据（第 100 个 `net-tools` 已由 41-artifact 无重编 merge [`34007657490`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007657490) 纳入 merged candidate；第 99 个 `psmisc` 已由 40-artifact 无重编 merge [`34006620790`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006620790) 纳入；第 98 个 `lzop` 已由 39-artifact 无重编 merge [`34004352420`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004352420) 纳入；第 97 个 `lz4` 已由 38-artifact 无重编 merge `34003078163` 纳入；第 96 个 `pigz` 已由 37-artifact 无重编 merge `34001947527` 纳入；此前第 95 个 `xxhash` 已由 36-artifact 无重编 merge `34000394791` 纳入）；其余 12 个通用 package 是 immutable target catalogue provider：`ca-certificates`、`libatomic-1`、
...
Cardputer 应用不属于本 release。`pigz` 已计入 96 个 source-build package，并由无重编 merge 验证 exact artifact/IPK hash；它仍未签名、发布、部署或在设备执行。
...
`logrotate` 的 source lock、私有命令路径、immutable `libpopt` reuse 和独立
`log-maintenance-tools` Actions batch 已由 run
[`33991963284`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33991963284) 实际验证：从锁定
cache 交叉构建的 `logrotate_3.22.0-1_riscv64.ipk` 通过 feed verification，并上传 artifact
[`9976939148`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33991963284/artifacts/9976939148)
（90,128,669 bytes；zip SHA-256 `a9e906c17ec972d30547e26efbb8c05bf2afc0f5ad2479b7c4efb828026de4b1`）。
它计入 87，并已由 28-artifact no-recompile merge run
[`33992249214`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33992249214) 纳入 merged candidate：
run 下载并比对全部 28 个 compatible artifact 的 IPK hash，只重建 `Packages` 索引、验证 runtime closure 和
445 个 non-ABI dynamic objects 的 target-runtime coverage；SDK-build 与 package-build job 都是 skipped。最终
merged unsigned artifact 为
[`9977025485`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33992249214/artifacts/9977025485)
（`tdvp-k230-r10-merged-unsigned-81d3815…`，196,453,342 bytes；zip SHA-256
`280fae94c6c42222b79a036456db3ecc84f3355b439993e90ef96c155454d874`）。CI 的验收只检查
package payload，绝不执行该命令、传入路径或读写、重命名、压缩、删除/轮转任何日志。
`jo` 的 source lock、私有命令路径和独立 `json-construction-tools` Actions batch 已由 run
[`33992855036`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33992855036) 实际验证：从锁定 cache
...
`ed` 的 source lock、私有命令路径和 `line-editor-tools` Actions batch 已由 run
[`33993563150`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33993563150) 实际验证：从锁定 cache
交叉构建的 `ed_1.20.2-1_riscv64.ipk` 通过 feed verification，并上传 artifact
[`9977399730`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33993563150/artifacts/9977399730)
（90,119,822 bytes；zip SHA-256 `bb44609d5f990773e17464851bd3f398e40270562c3fbd38ba87aec4abc99f65`）。
它计入 89，并已由 30-artifact no-recompile merge run
[`33993808518`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33993808518) 纳入 merged candidate：run 下载并比对全部 30 个
compatible artifact 的 IPK hash，只重建 `Packages` 索引、验证 runtime closure 和 445 个 non-ABI dynamic objects 的
...
它计入 90，并已由 31-artifact no-recompile merge run
[`33994729377`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33994729377) 纳入 merged candidate：run 下载并比对全部 31 个
compatible artifact 的 IPK hash，只重建 `Packages` 索引、验证 runtime closure 和 445 个 non-ABI dynamic objects 的
target-runtime coverage；SDK-build 与 package-build job 都是 skipped。最终 merged unsigned artifact 为
[`9977771246`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33994729377/artifacts/9977771246)
（196,596,936 bytes；zip SHA-256 `e3d9ca0bd9e6e1f8bab58c4409e5c5118c0082eda4f7b36a04355026df6af3d2`）。CI 未执行 `cpio`、传入 archive 或 filesystem path，也未创建、解包、读取或写入 archive payload。
`time` 的 source lock、私有命令路径和独立 `process-timing-tools` Actions batch 已由 run
[`33995251958`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33995251958) 实际验证：从锁定 cache
交叉构建的 `time_1.9-1_riscv64.ipk` 通过 feed verification，并上传 artifact
[`9977896731`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33995251958/artifacts/9977896731)
（90,100,557 bytes；zip SHA-256 `7aa9077c25588d06e70d62c109cd64606dfa6565e768211654e7fadcc7bacb75`）。
它计入 91，并已由 32-artifact no-recompile merge run [`33995532939`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33995532939) 纳入 merged candidate：run 下载并比对全部 32 个 compatible artifact 的 IPK hash，只重建 `Packages` 索引、验证 runtime closure 和 445 个 non-ABI dynamic objects 的 target-runtime coverage；SDK-build 与 package-build job 都是 skipped。最终 merged unsigned artifact 为 [`9978007846`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33995532939/artifacts/9978007846)（196,610,040 bytes；zip SHA-256 `e8095503179d3ce2aaa0d22dea7b1edb7c38f32a1eb85e001dfe516978cbdd0e`）。CI 未执行 `time` 或为其启动被计时的子命令。
