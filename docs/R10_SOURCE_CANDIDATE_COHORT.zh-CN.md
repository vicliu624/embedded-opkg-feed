...
| 32 | `rhash`（GitHub Actions source batch 已通过，待无重编 merge/实机） | 1.4.4 | `checksum-tools` 只允许私有 `rhash` ELF 与 `tdvp-rhash`。run [`33998249379`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33998249379) 用锁定 RHash 来源实际构建一个 IPK，并通过 source cache、RISC-V ELF、runtime closure、deny overlay、feed verification 和 445 个 non-ABI dynamic object 的 target-runtime coverage；source artifact [`9978739567`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33998249379/artifacts/9978739567) 为 90,288,593 bytes，SHA-256 `85f7c5752bfadd140bd9a34c9c7cc7069890461e23f57b5f5e63e9b670ae65c3`。recipe 显式关闭 gettext/OpenSSL、静态链接 private `librhash` implementation 并关闭 shared library；不构建或发布 runtime provider。CI 未执行该命令、传入文件路径或读取 payload。 |
