...
| Parallel gzip (prepared) | `pigz` | The official [pigz 2.8 source release](https://zlib.net/pigz/) is the current upstream release and its streamed SHA-256 is `eb872b4f0e1f0ebe59c9f7bd8c506c4204893ba6a8492de31df416f0d5170fd0`, identical to the Buildroot 2025.02.1 lock. The original zlib.net transfer endpoint returned 12,031 bytes with hash `8873b211638620c1cbb32ed01a1d6e03ea928a8842566216d2d8040248b97a0c` in GitHub Actions run [`34001374220`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001374220), so source-cache rejected it before compilation with exit 69. The source lock uses the OmniOS public HTTPS mirror only after independently streaming its 121,304-byte archive to the same upstream/Buildroot hash; this is an exact-content transport substitution, not a source replacement or a hash relaxation. The recipe makes one private `pigz` ELF available only as `tdvp-pigz`, never replaces BusyBox `gzip`, and relies only on the exact immutable `libz` provider selected by its audited ELF closure. No zlib, headers, static archive, or other shared-runtime ABI can enter the IPK. GitHub Actions may build/audit the ELF but must never invoke pigz, give it content, compress data, or extract data. Compression and CRC are not content authentication. This row has no successful source-batch or merge artifact until those two GitHub Actions gates pass. |
