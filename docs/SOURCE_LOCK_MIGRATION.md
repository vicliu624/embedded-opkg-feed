...
| Line editing | `ed` | GitHub Actions source batch [`33993563150`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33993563150) passed the reviewed GNU command leaf: runner-only host-lzip unpacked the locked archive, then only private `ed` with the `tdvp-ed` frontend was packaged. No shared provider was introduced. The successful source artifact awaits a no-recompile merge; CI did not invoke it or give it a file. |
