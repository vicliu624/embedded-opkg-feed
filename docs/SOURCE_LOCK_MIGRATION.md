...
| JSON construction | `jo` | The reviewed standalone command leaf is prepared: it packages only private `jo` with the `tdvp-jo` frontend and introduces no shared-runtime provider. GitHub Actions must prove source cache, RISC-V ELF, closure, and deny overlay before a no-recompile merge; CI must never invoke it or provide JSON input. |
