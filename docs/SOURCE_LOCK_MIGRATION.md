...
| Legacy network tools (source-locked; Actions pending) | `net-tools` | net-tools 2.10 exactly matches the fixed Buildroot 2025.02.1 recipe and official archive SHA-256 `b262435a5241e89bfa51c3cabd5133753952f7a7b7b93f32e08cb9d96f580d69`. It is a command-only candidate: the transaction can retain only selected private command ELFs and distinct `tdvp-*` frontends, never a BusyBox path, configuration/service, header, archive, pkg-config metadata, or shared-runtime ABI. CI must compile/audit only: it cannot invoke a utility, read an interface, supply interface/address/route/device input, open a connection, or alter network configuration. Source build, no-recompile merge, signing, publication, deployment, and explicitly authorized device network lifecycle validation remain pending. |
