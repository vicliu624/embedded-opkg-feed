...
| Log maintenance | `logrotate` | The reviewed command-only leaf is prepared: it reuses the immutable target `libpopt` provider, disables SELinux/ACL, and packages only a private `logrotate` ELF with the `tdvp-logrotate` frontend. No `/etc/logrotate.conf`, `/etc/logrotate.d`, timer or daemon is included. GitHub Actions must prove source cache, RISC-V ELF, closure, and deny overlay before a no-recompile merge; CI must never invoke it or rotate a log. |
