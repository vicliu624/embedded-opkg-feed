...
| Fast non-cryptographic integrity | `xxhash` (planned source batch) | Buildroot 2025.02.1 locks xxHash 0.8.3 and its GitHub tag archive SHA-256 is `aae608dfe8213dfd05d909a57718ef82f30722c392344583d3f39050c7f29a80`. The reviewed private transaction builds/installs only `xxhsum`, which links its implementation objects directly; it does not build or install `libxxhash`, headers, or pkg-config metadata. `xxhsum` is restricted to controlled deduplication and non-security prefiltering, never authentication, signatures, or security decisions. This row is recipe/provenance evidence only until a GitHub Actions source batch and a later no-recompile merge both succeed. |
