...
| Bandwidth monitoring | `bwm-ng` | GitHub Actions source batch [`33997001920`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33997001920) passed the reviewed private command leaf and produced unsigned artifact [`9978397329`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33997001920/artifacts/9978397329). Its optional terminal view consumes only the immutable target `libncursesw` provider; it builds and publishes no runtime provider. The required no-recompile hash merge remains pending, so this is source-build evidence rather than candidate admission. CI did not invoke it, read procfs, observe a network interface, or observe disk I/O. |
