...
| Parallel LZO (locked; source batch pending) | `lzop` | The official HTTPS LZO 2.10 and lzop 1.04 archives stream to SHA-256 `c0f892943208266f9b6543b3ae308fab6284c5c90e627931446fb49b4221a072` and `7e72b62a8a60aff5200a047eea0773a8fb205caf7acbe1774d95147f305a2f41`, respectively, exactly matching Buildroot 2025.02.1. The reviewed private transaction enables `BR2_PACKAGE_LZOP` and forces `LZO_CONF_OPTS=-DENABLE_SHARED=OFF -DENABLE_STATIC=ON`: LZO is build-time implementation only, while the generic helper may extract only `lzop` below `/usr/libexec/tdvp-lzop` with `tdvp-lzop` frontend. It must never publish `liblzo2.so`, `liblzo2.a`, headers, pkg-config metadata, or an ordinary firmware path. A GitHub Actions source batch must prove locked cache, RISC-V ELF, deny-overlay, feed verification, and runtime closure; any surviving `liblzo2` runtime dependency must fail closed. CI must not invoke the codec, give it content, compress data, or extract data. LZO/LZOP framing and checksums are not content authentication. |
