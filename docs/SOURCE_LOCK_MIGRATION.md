...
| Fast LZ4 (prepared) | `lz4` | The official [LZ4 v1.10.0 tag archive](https://github.com/lz4/lz4/releases/tag/v1.10.0) is the current upstream release and its streamed SHA-256 is `537512904744b35e232912055ccf8ec66d768639ff3abe5788d90d792ec5f48b`, identical to Buildroot 2025.02.1. The candidate enables only Buildroot's LZ4 program option and passes `LZ4_MAKE_OPTS=BUILD_SHARED=no`, then extracts only private `lz4`, `lz4c`, `unlz4`, and `lz4cat` command paths with distinct `tdvp-` frontends. It must never emit `liblz4.so`, `liblz4.a`, headers, pkg-config metadata, or an ordinary firmware command path. GitHub Actions may build/audit the ELF closure but must never invoke the codec, give it content, compress data, or extract data. LZ4 framing and checksums are not content authentication. This row has no source-batch or merge artifact until both GitHub Actions gates pass. |
