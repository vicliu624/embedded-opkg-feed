...
| Filesystem event inspection | `inotify-tools` | The reviewed private command leaf is prepared: disable shared output, statically link the private libinotifytools implementation into the commands, and package only `inotifywait`/`inotifywatch`; no `libinotifytools` or headers. GitHub Actions must prove source cache, RISC-V ELF, closure, and deny overlay before a no-recompile merge. CI must never start a watcher or observe a path. |
