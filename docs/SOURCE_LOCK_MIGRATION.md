...
| Filesystem search (in progress) | `findutils` | The locked current GNU 4.10.0 source archive was streamed from the official HTTPS endpoint and matched its Buildroot SHA-256 on 2026-09-06. Its private candidate will retain only `find`/`xargs` ELF files with `tdvp-find`/`tdvp-xargs` frontends; it will not package `locate`, `updatedb`, or a shared provider. The Debian security tracker records known findutils issues as resolved. The planned GitHub Actions batch will restore the immutable SDK cache and build/audit only; it must not call `find`, supply a filesystem path, invoke `xargs`, or start a child command. It remains unadmitted until a source batch and later no-recompile merge pass. |
