...
| Line editing | `ed` | The reviewed GNU command leaf is prepared: runner-only host-lzip unpacks the locked archive, then only private `ed` with the `tdvp-ed` frontend is packaged. No shared provider is introduced. GitHub Actions must prove source cache, RISC-V ELF, closure, and deny overlay before a no-recompile merge; CI must never invoke it or give it a file. |
