...
| Process inspection suite (source-locked; Actions pending) | `psmisc` | psmisc 23.7 is the official latest release; the locked SourceForge HTTPS archive streams to SHA-256 `58c55d9c1402474065adae669511c191de374b0871eec781239ab400b907c327`. This is an explicit upstream-version-delta candidate: Buildroot 2025.02.1's `psmisc.mk` remains only the reviewed cross-build logic while a guarded make variable changes `PSMISC_VERSION` from its baseline 23.5 to the locked 23.7 release. Because the baseline `psmisc.hash` has no 23.7 row, the transaction supplies a one-row override only after its archive name and SHA-256 exactly match `source.lock`; normal Buildroot hash verification remains enabled and the original manifest is restored byte-for-byte on both success and failure. The prospective IPK may contain only private `fuser`, `killall`, `pslog`, `prtstat`, and `pstree` ELF files and distinct `tdvp-*` frontends. It consumes the exact immutable `libncursesw (= 6.4-20230603-1)` provider, publishes neither a shared ABI nor normal firmware paths, and excludes `peekfd` because 23.7 has no RISC-V implementation. CI is forbidden to invoke the commands, inspect procfs, pass PIDs/names/paths/sockets, signal processes, or traverse live process trees. Source build, no-recompile merge, signing, publication, deployment, and device lifecycle validation are all still pending. |
