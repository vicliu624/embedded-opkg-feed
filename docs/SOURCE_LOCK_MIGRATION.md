...
| I2C hardware inspection | `i2c-tools` | The reviewed static-only leaf design is now prepared: disable Python/py-smbus, force the five commands to link private `libi2c.a`, and package neither `libi2c.a` nor `libi2c.so`. GitHub Actions must prove that boundary, RISC-V ELF, closure, and deny overlay before a no-recompile merge. It must never probe or write a bus in CI. |
