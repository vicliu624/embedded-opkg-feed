...
| 进程计时 | `time` | GitHub Actions source batch [`33995251958`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33995251958) 已通过经审查的 GNU command leaf：K230 的 MMU/dynamic-library/BusyBox-show-others profile 满足上游条件，随后只封装私有 `time` 与 `tdvp-time` frontend，不引入 shared provider。成功 source artifact 正待无重编 merge；CI 未执行它或为其启动被计时的子命令。 |
