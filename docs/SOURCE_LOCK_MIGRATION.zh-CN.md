...
| 批次 | package | 先决条件与原因 |
| --- | --- | --- |
| 调试/网络诊断 | `gdbserver`、`ethtool` | GitHub Actions source batch [`33988534267`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33988534267) 已用锁定的 Buildroot 2025.02.1 来源及私有 TDVP 前端通过。`gdbserver` 保持 server-only，未修改 SDK host `debug-root`；`ethtool` 保持 no-netlink/no-libmnl 边界。两个已验证 source IPK 现只等待 no-recompile merge；CI 未启动 server 或修改网络接口。 |
