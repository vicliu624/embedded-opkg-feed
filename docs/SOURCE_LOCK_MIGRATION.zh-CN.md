...
| 进程诊断套件（已锁定来源；待 Actions） | `psmisc` | psmisc 23.7 是官方当前 release；锁定的 SourceForge HTTPS archive 流式 SHA-256 是 `58c55d9c1402474065adae669511c191de374b0871eec781239ab400b907c327`。这是明确的上游版本增量候选：Buildroot 2025.02.1 的 `psmisc.mk` 只保留为受审交叉构建逻辑，受限 Make variable 将它的基线 `PSMISC_VERSION` 23.5 改为锁定的 23.7。因为基线 `psmisc.hash` 没有 23.7 行，事务仅在 archive 名称和 SHA-256 与 `source.lock` 精确相同后才提供一行 override；正常 Buildroot 哈希校验保持开启，原清单无论成功或失败均按字节还原。未来 IPK 只能包含 private `fuser`、`killall`、`pslog`、`prtstat`、`pstree` ELF 和不同的 `tdvp-*` frontend；它精确使用 immutable `libncursesw (= 6.4-20230603-1)`，不发布 shared ABI 或普通 firmware path。由于 23.7 没有 RISC-V 实现，`peekfd` 明确排除。CI 禁止运行命令、检查 procfs、传入 PID/名称/路径/socket、发送信号或遍历真实进程树。source build、无重编 merge、签名、发布、部署和实机生命周期验证都仍待完成。 |
