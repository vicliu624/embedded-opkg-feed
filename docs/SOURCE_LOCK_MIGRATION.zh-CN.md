...
| 进程诊断套件（已 source-build；待无重编 merge） | `psmisc` | psmisc 23.7 官方 archive 的锁定 SHA-256 是 `58c55d9c1402474065adae669511c191de374b0871eec781239ab400b907c327`。这是上游版本增量：Buildroot 2025.02.1 的 `psmisc.mk` 只用作受审交叉构建逻辑，受限 `PSMISC_VERSION=23.7` 覆盖其基线 23.5。因为基线 `psmisc.hash` 没有 23.7 行，事务仅在 archive 名称和 SHA-256 与 `source.lock` 唯一精确匹配后临时提供一行 override；普通 Buildroot 哈希校验保持开启，原清单无论成功或失败均按字节还原。GitHub Actions source batch [`34006256552`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552) 恢复而非重建 SDK，从锁定 cache 构建 `psmisc_23.7-1_riscv64.ipk`，通过 source cache、RISC-V ELF、runtime closure、deny overlay、feed verification 与 445-object target coverage；SDK-base 和 merge job 均 skipped。unsigned source artifact [`9981057795`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552/artifacts/9981057795) 为 90,137,169 bytes，GitHub SHA-256 `db9fcdbde1a26de09dcb5270f53213d9c5d514c1a50521243e7442eb7525f58a`。IPK 只能包含 private `fuser`、`killall`、`pslog`、`prtstat`、`pstree` ELF 和不同的 `tdvp-*` frontend；精确使用 immutable `libncursesw (= 6.4-20230603-1)`，不发布 shared ABI 或普通 firmware path，排除无 RISC-V 实现的 `peekfd`。CI 禁止运行命令、检查 procfs、传入 PID/名称/路径/socket、发送信号或遍历真实进程树。无重编 merge、签名、发布、部署和实机生命周期验证仍待完成。 |
