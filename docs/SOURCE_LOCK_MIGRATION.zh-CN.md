...
| 日志维护 | `logrotate` | 经审查的 command-only leaf 已准备：精确复用 immutable target `libpopt` provider，禁用 SELinux/ACL，只封装私有 `logrotate` ELF 与 `tdvp-logrotate` frontend。不包含 `/etc/logrotate.conf`、`/etc/logrotate.d`、timer 或 daemon。GitHub Actions 必须在无重编 merge 前证明 source cache、RISC-V ELF、closure 与 deny overlay；CI 不得执行它或轮转日志。 |
