...
| 文件系统搜索（进行中） | `findutils` | 锁定的当前 GNU 4.10.0 source archive 已于 2026-09-06 从官方 HTTPS 端点流式下载，并匹配其 Buildroot SHA-256。其私有候选只保留 `find`/`xargs` ELF 及 `tdvp-find`/`tdvp-xargs` frontend，不封装 `locate`、`updatedb` 或 shared provider。Debian security tracker 将已知 findutils 问题记录为 resolved。计划中的 GitHub Actions batch 只恢复 immutable SDK cache 并构建/审计；不得调用 `find`、传入 filesystem path、调用 `xargs` 或启动子命令。它在 source batch 与后续 no-recompile merge 均通过前仍未准入。 |
