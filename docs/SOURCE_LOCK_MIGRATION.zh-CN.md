...
| 并行 gzip（已准备） | `pigz` | 官方 [pigz 2.8 source release](https://zlib.net/pigz/) 是当前上游 release；流式复算 SHA-256 为 `eb872b4f0e1f0ebe59c9f7bd8c506c4204893ba6a8492de31df416f0d5170fd0`，与 Buildroot 2025.02.1 锁定值一致。GitHub Actions run [`34001374220`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001374220) 从原 zlib.net 传输端得到的 12,031 bytes 内容 SHA-256 是 `8873b211638620c1cbb32ed01a1d6e03ea928a8842566216d2d8040248b97a0c`，因此 source-cache 在实际编译前以 exit 69 拒绝。`source.lock` 仅在独立流式复算 OmniOS 公共 HTTPS mirror 的 121,304-byte archive 仍等于上游/Buildroot 同一哈希后，才切换传输端；这不是源码替换，也不是放宽散列。recipe 仅把一个私有 `pigz` ELF 以 `tdvp-pigz` 提供，绝不替换 BusyBox `gzip`，其经审计的 ELF closure 只能消费 immutable `libz` 精确 provider。不允许 zlib、headers、静态 archive 或其他 shared-runtime ABI 进入 IPK。GitHub Actions 只能构建/审计 ELF，绝不运行 pigz、传入内容、压缩或解压数据；压缩和 CRC 不是内容认证。本行尚无成功 source-batch 或 merge artifact，必须先通过这两个门才会产生候选证据。 |
