...
| 并行 gzip（已 source-build；待 merge） | `pigz` | 官方 [pigz 2.8 source release](https://zlib.net/pigz/) 是当前上游 release；SHA-256 `eb872b4f0e1f0ebe59c9f7bd8c506c4204893ba6a8492de31df416f0d5170fd0` 与 Buildroot 2025.02.1 锁定值一致。run [`34001374220`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001374220) 从原 zlib.net 传输端取得 12,031 bytes、SHA-256 `8873b211638620c1cbb32ed01a1d6e03ea928a8842566216d2d8040248b97a0c` 的错误内容，所以 source-cache 在实际编译前以 exit 69 拒绝。仅在 OmniOS 公共 HTTPS mirror 的 121,304-byte archive 被独立流式复算为同一上游/Buildroot hash 后，`source.lock` 才切换传输端；这不是源码替换或放宽散列。GitHub Actions source batch [`34001592645`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001592645) 恢复（未重建）SDK，用 `riscv64-unknown-linux-gnu-gcc` 交叉编译 `pigz`，生成 `pigz_2.8-1_riscv64.ipk`，并通过 locked source、RISC-V ELF、closure、deny-overlay、feed-verification 与 445-object target-coverage gate。unsigned artifact [`9979676644`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001592645/artifacts/9979676644) 为 90,155,403 bytes，GitHub SHA-256 `a022179609394e636be83251319b31af957d52fd47366fcf78bc15058c3791c4`。recipe 仅以 `tdvp-pigz` 提供 private ELF，绝不替换 BusyBox `gzip`，只消费 immutable `libz` 精确 provider；禁止 zlib、headers、静态 archive 或其他 shared-runtime ABI 进入 IPK。CI 未运行 pigz、传入内容、压缩或解压数据；压缩和 CRC 不是内容认证。artifact 尚未签名，仍需无重编 merge 和实机生命周期验证。 |
