...
| 旧式网络工具（已锁定来源；待 Actions） | `net-tools` | net-tools 2.10 与固定 Buildroot 2025.02.1 的 recipe 和官方 archive SHA-256 `b262435a5241e89bfa51c3cabd5133753952f7a7b7b93f32e08cb9d96f580d69` 精确一致。它是 command-only 候选：事务只能保留 selected private command ELF 与不同的 `tdvp-*` frontend，绝不包含 BusyBox path、配置/服务、headers、archive、pkg-config metadata 或 shared-runtime ABI。CI 只能编译/审计，不得执行命令、读取接口、传入 interface/address/route/device、建立连接或修改网络配置。source build、无重编 merge、签名、发布、部署和获得明确授权后的设备侧网络生命周期验证均待完成。 |
