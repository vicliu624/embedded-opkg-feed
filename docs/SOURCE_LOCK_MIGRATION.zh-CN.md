...
| 带宽监视 | `bwm-ng` | GitHub Actions source batch [`33997001920`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33997001920) 已通过经审查的私有 command leaf，并产生 unsigned artifact [`9978397329`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33997001920/artifacts/9978397329)。其可选 terminal view 只消费 immutable target `libncursesw` provider，不构建或发布 runtime provider。所需的 no-recompile hash merge 尚未完成，所以这只是 source-build evidence，不是 candidate 准入。CI 未执行监视器、读取 procfs、观察网络接口或观察磁盘 I/O。 |
