...
| JSON 构造 | `jo` | 经审查的独立 command leaf 已准备：只封装私有 `jo` 与 `tdvp-jo` frontend，不引入 shared-runtime provider。GitHub Actions 必须在无重编 merge 前证明 source cache、RISC-V ELF、closure 与 deny overlay；CI 不得执行它或传入 JSON input。 |
