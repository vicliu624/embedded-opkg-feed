...
| 非加密快速完整性 | `xxhash`（计划 source batch） | Buildroot 2025.02.1 锁定 xxHash 0.8.3；官方 GitHub tag archive 的 SHA-256 是 `aae608dfe8213dfd05d909a57718ef82f30722c392344583d3f39050c7f29a80`。经审查的私有 transaction 只构建/安装直接链接 implementation object 的 `xxhsum`，不构建或安装 `libxxhash`、headers 或 pkg-config metadata。`xxhsum` 仅限受控去重和非安全预筛选，绝不用于认证、签名或安全决策。在 GitHub Actions source batch 与后续无重编 merge 都成功前，本行只是 recipe/provenance 证据。 |
