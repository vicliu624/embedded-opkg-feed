...
| 快速 LZ4（已准备） | `lz4` | 官方 [LZ4 v1.10.0 tag archive](https://github.com/lz4/lz4/releases/tag/v1.10.0) 是当前上游 release；流式 SHA-256 `537512904744b35e232912055ccf8ec66d768639ff3abe5788d90d792ec5f48b` 与 Buildroot 2025.02.1 一致。candidate 只开启 Buildroot 的 LZ4 program option，并传入 `LZ4_MAKE_OPTS=BUILD_SHARED=no`，随后只提取 private `lz4`、`lz4c`、`unlz4`、`lz4cat` command 与不同的 `tdvp-` frontend。不得生成 `liblz4.so`、`liblz4.a`、headers、pkg-config metadata 或普通 firmware command path。GitHub Actions 可以构建/审计 ELF closure，但绝不运行 codec、传入内容、压缩或解压数据。LZ4 frame/checksum 不是内容认证。本行在两个 GitHub Actions gate 都通过前没有 source-batch 或 merge artifact。 |
