...
| 行编辑 | `ed` | 经审查的 GNU command leaf 已准备：runner-only host-lzip 解包锁定 archive，随后只封装私有 `ed` 与 `tdvp-ed` frontend，不引入 shared provider。GitHub Actions 必须在无重编 merge 前证明 source cache、RISC-V ELF、closure 与 deny overlay；CI 不得执行它或传入文件。 |
