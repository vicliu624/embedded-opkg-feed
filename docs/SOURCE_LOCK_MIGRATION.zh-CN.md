...
| 文件系统事件检查 | `inotify-tools` | 经审查的私有 command leaf 已准备：禁用 shared output、把私有 `libinotifytools` implementation static link 进命令，并且只封装 `inotifywait`/`inotifywatch`；不含 `libinotifytools` 或头文件。GitHub Actions 必须在无重编 merge 前证明 source cache、RISC-V ELF、closure 与 deny overlay。CI 不得启动 watcher 或观察路径。 |
