...
| 并行 gzip（已准备） | `pigz` | 官方 [pigz 2.8 source release](https://zlib.net/pigz/) 是当前上游 release；流式复算 SHA-256 为 `eb872b4f0e1f0ebe59c9f7bd8c506c4204893ba6a8492de31df416f0d5170fd0`，与 Buildroot 2025.02.1 锁定值一致。recipe 仅把一个私有 `pigz` ELF 以 `tdvp-pigz` 提供，绝不替换 BusyBox `gzip`，其经审计的 ELF closure 只能消费 immutable `libz` 精确 provider。不允许 zlib、headers、静态 archive 或其他 shared-runtime ABI 进入 IPK。GitHub Actions 只能构建/审计 ELF，绝不运行 pigz、传入内容、压缩或解压数据；压缩和 CRC 不是内容认证。本行仅记录 source-lock/static-policy 准备，必须先通过 GitHub Actions source batch 和随后无重编 merge，才会产生候选 artifact 证据。 |
