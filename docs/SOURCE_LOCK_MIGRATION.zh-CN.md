...
| 并行 LZO（已锁定；待 source batch） | `lzop` | 官方 HTTPS LZO 2.10 与 lzop 1.04 archive 的流式 SHA-256 分别为 `c0f892943208266f9b6543b3ae308fab6284c5c90e627931446fb49b4221a072`、`7e72b62a8a60aff5200a047eea0773a8fb205caf7acbe1774d95147f305a2f41`，与 Buildroot 2025.02.1 精确一致。经审查的 private transaction 启用 `BR2_PACKAGE_LZOP`，并强制 `LZO_CONF_OPTS=-DENABLE_SHARED=OFF -DENABLE_STATIC=ON`：LZO 只是 build-time implementation，generic helper 只可把 `lzop` 提取到 `/usr/libexec/tdvp-lzop`，由 `tdvp-lzop` frontend 使用。它绝不可发布 `liblzo2.so`、`liblzo2.a`、headers、pkg-config metadata 或普通 firmware path。GitHub Actions source batch 必须证明 locked cache、RISC-V ELF、deny-overlay、feed verification 与 runtime closure；任何残留 `liblzo2` runtime dependency 都必须 fail-closed。CI 不得运行 codec、传入内容、压缩或解压数据。LZO/LZOP frame/checksum 不是内容认证。 |
