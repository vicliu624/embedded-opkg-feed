...
22. The `net-tools 2.10` source-lock candidate uses the exact Buildroot 2025.02.1 source version and digest. It exposes only the reviewed private `tdvp-arp`, `tdvp-ifconfig`, `tdvp-ipmaddr`, `tdvp-iptunnel`, `tdvp-mii-tool`, `tdvp-nameif`, `tdvp-netstat`, `tdvp-plipconfig`, `tdvp-rarp`, `tdvp-route`, and `tdvp-slattach` frontends. CI cross-builds and audits the payload only; it neither invokes the tools nor queries or changes network state. Source, ELF, and closure checks must pass before the unsigned artifact can join the all-artifact merge; then
