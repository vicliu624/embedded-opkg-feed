...
21. Completed psmisc 23.7 source-build admission: GitHub Actions source batch [`34006256552`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552) restored—not rebuilt—the SDK, supplied a one-row temporary `psmisc.hash` override only after matching its filename and SHA-256 to `source.lock`, retained normal Buildroot hash verification, and restored the original manifest byte-for-byte. It built `psmisc_23.7-1_riscv64.ipk` from the locked cache, passed source/ELF/closure/deny-overlay/feed/445-object target coverage gates, and uploaded unsigned artifact [`9981057795`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552/artifacts/9981057795) with GitHub SHA-256 `db9fcdbde1a26de09dcb5270f53213d9c5d514c1a50521243e7442eb7525f58a`; the SDK-base and merge jobs were skipped. It exposes only private `tdvp-*` frontends for `fuser`, `killall`, `pslog`, `prtstat`, and `pstree`, consumes only immutable `libncursesw`, and excludes RISC-V-unsupported `peekfd`. CI did not invoke the commands or inspect procfs. The all-artifact no-recompile merge, signing, publication, and device lifecycle validation remain pending; then
