...
mirror, must fail the build. The only version-delta exception is a reviewed
one-row temporary package `.hash` override: its filename and SHA-256 must
exactly match exactly one `source.lock` artifact, Buildroot's ordinary hash
check remains enabled, and the original `.hash` is restored byte-for-byte on
both success and failure. The Buildroot transaction changes the SDK output
...
13. Completed: GitHub Actions source batch
...
18. Completed source admission: GitHub Actions source batch [`34001592645`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001592645) restored the SDK without rebuilding it, cross-compiled the locked pigz 2.8 command, and uploaded `pigz_2.8-1_riscv64.ipk` in unsigned artifact [`9979676644`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001592645/artifacts/9979676644). Its lock retained the upstream/Buildroot SHA-256 after the original transfer endpoint was rejected by source-cache. No-recompile merge [`34001947527`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001947527) compared all 37 artifact manifests/IPK hashes, rebuilt the index, and passed closure/445-object coverage without compiling; the SDK-base and source-build jobs were skipped, and merged artifact [`9979808940`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34001947527/artifacts/9979808940) was uploaded. It only consumes immutable `libz`, provides private `tdvp-pigz`, never replaces BusyBox `gzip`, and CI never invoked it or gave it content. It is unsigned and still requires device lifecycle validation; then
19. Completed source admission: GitHub Actions source batch [`34002789451`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34002789451) restored the SDK without rebuilding it, built the locked Buildroot LZ4 1.10.0 `lib/` and `programs/` directories with `BUILD_SHARED=no`, and uploaded `lz4_1.10.0-1_riscv64.ipk` in unsigned artifact [`9980027622`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34002789451/artifacts/9980027622). It provides only private `tdvp-lz4` command frontends, not `liblz4` ABI, headers, pkg-config metadata, or an ordinary firmware command path. No-recompile merge [`34003078163`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34003078163) compared all 38 artifact manifests/IPK hashes, rebuilt the index, and passed feed verification, runtime closure, and 445-object target coverage without compiling; the SDK-base and source-build jobs were skipped, and merged artifact [`9980152139`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34003078163/artifacts/9980152139) was uploaded. CI never invoked LZ4 or gave it content. It is unsigned and still requires device lifecycle validation; then
20. Completed source admission: GitHub Actions source batch [`34004037276`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004037276) restored the SDK without rebuilding it, configured locked LZO 2.10 with `-DENABLE_SHARED=OFF -DENABLE_STATIC=ON`, linked `liblzo2.a`, and cross-built private `lzop_1.04-1_riscv64.ipk` in unsigned artifact [`9980420311`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004037276/artifacts/9980420311). Its command is private `tdvp-lzop`, never a `liblzo2` ABI provider, header package, pkg-config provider, or ordinary firmware command path. No-recompile merge [`34004352420`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004352420) compared all 39 artifact manifests/IPK hashes, rebuilt the index, and passed feed verification, runtime closure, and 445-object target coverage without compiling; the SDK-base and source-build jobs were skipped, and merged artifact [`9980529319`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004352420/artifacts/9980529319) was uploaded. CI never invoked LZOP or gave it content. It is unsigned and still requires device lifecycle validation; then
21. Completed psmisc 23.7 source admission and merge: source batch [`34006256552`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552) restored—not rebuilt—the SDK, admitted its one-row temporary `psmisc.hash` override only after exact `source.lock` filename/SHA-256 matching, retained normal Buildroot hash verification, restored the original manifest byte-for-byte, and built `psmisc_23.7-1_riscv64.ipk`. It uploaded unsigned artifact [`9981057795`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552/artifacts/9981057795) (GitHub SHA-256 `db9fcdbde1a26de09dcb5270f53213d9c5d514c1a50521243e7442eb7525f58a`). No-recompile merge [`34006620790`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006620790) compared all 40 input artifact manifests/IPK hashes, rebuilt the index, and passed feed verification, runtime closure, and 445-object target coverage without compiling; SDK-base and package-build jobs were skipped. It uploaded merged unsigned artifact [`9981196020`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006620790/artifacts/9981196020), 197,245,213 bytes with GitHub SHA-256 `3abcbefac768ad21ebe49df0d54050b0d191e2891687e495dc0899a19d53e91d`. The package exposes only private `tdvp-*` frontends for `fuser`, `killall`, `pslog`, `prtstat`, and `pstree`, consumes only immutable `libncursesw`, and excludes RISC-V-unsupported `peekfd`; CI did not invoke it or inspect procfs. The artifacts remain unsigned and require device lifecycle validation; then
22. Completed `net-tools 2.10` source admission and merge: source batch [`34007398511`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007398511) restored—not rebuilt—the SDK, used the exact Buildroot 2025.02.1 source version/digest, cross-built `net-tools_2.10-1_riscv64.ipk`, and passed source, RISC-V ELF, dependency-closure, feed, and 445-object target-coverage gates. It uploaded unsigned artifact [`9981425511`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007398511/artifacts/9981425511), 90,328,415 bytes with GitHub SHA-256 `9761e314772218eed4b4ee9c6fd14ef50fadfd158289b3a443fc35c80c8efc25`; SDK-base and merge jobs were skipped. No-recompile merge [`34007657490`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007657490) compared all 41 input artifact manifests/IPK hashes, rebuilt the index, and passed feed verification, runtime closure, and 445-object target coverage without compiling; SDK-base and package-build jobs were skipped. It uploaded merged unsigned artifact [`9981521824`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007657490/artifacts/9981521824), 197,486,158 bytes with GitHub SHA-256 `061756699b615cc167773297cec41b03562588eba3bd296aeefa3202f4f52ba2`. The IPK exposes only the reviewed private `tdvp-arp`, `tdvp-ifconfig`, `tdvp-ipmaddr`, `tdvp-iptunnel`, `tdvp-mii-tool`, `tdvp-nameif`, `tdvp-netstat`, `tdvp-plipconfig`, `tdvp-rarp`, `tdvp-route`, and `tdvp-slattach` frontends. CI cross-built and audited only; it neither invoked the tools nor queried or changed network state. The artifacts are unsigned and still require publication authorization and device lifecycle validation; then
23. Emit source provenance/SBOM material for each release and bind source, SDK,
   and test evidence to the signed release; then
24. Introduce reviewed general libraries incrementally, retaining shared-runtime
