...
21. Source-lock psmisc 23.7 as an upstream-version-delta process-inspection candidate: use Buildroot 2025.02.1 `psmisc.mk` only as reviewed cross-build logic, pass one guarded `PSMISC_VERSION=23.7` make variable, and seed only the official `psmisc-23.7.tar.xz` source whose SHA-256 is `58c55d9c1402474065adae669511c191de374b0871eec781239ab400b907c327`. It may expose only private `fuser`, `killall`, `pslog`, `prtstat`, and `pstree` with distinct `tdvp-*` frontends, reuse only exact immutable `libncursesw`, and exclude RISC-V-unsupported `peekfd`. Its batch must restore rather than rebuild the SDK, compile/audit only, never inspect procfs or invoke the commands, and pass source, ELF, overlay and closure gates before an all-artifact no-recompile merge; then
