...
7. 进行中：只通过 GitHub Actions source batch 和后续 no-recompile merge 准入锁定的 static-only `i2c-tools` 硬件检查 leaf；必须禁用 Python/py-smbus 与 shared `libi2c`，CI 不得检查或变更任何硬件总线；
