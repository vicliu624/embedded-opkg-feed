...
对于 `buildroot-derived` 命令包，feed 会先把 `source.lock` 声明的**全部**已校验归档从 TDVP source cache 复制到每次构建专有的临时下载目录，再以该目录作为 `BR2_DL_DIR`，并把 Buildroot 限定为只可访问这个本地 primary site。这里既包括 target 源码，也包括该精确构建路径触发的 Buildroot host helper（例如解压 `.tar.lz` 所需的 host `lzip`）；后者不是“构建机自带工具”的隐式豁免，必须有自身的 URL、文件名和 SHA-256，写入同一 recipe 的来源锁。Buildroot recipe 的文件名或哈希与锁不一致、或它试图回退到网络时，构建必须失败。唯一的上游版本增量例外是经过审查的 package `.hash` 单行临时 override：其文件名和 SHA-256 必须与**唯一一条** `source.lock` artifact 精确一致，普通 Buildroot 哈希检查仍必须开启，且不论成功或失败都要按字节还原原 `.hash`。构建事务只会临时修改 SDK output 的配置；结束时按字节恢复原有 `.config` 和已有的 `.config.old`，绝不以 `olddefconfig`“规范化”维护者原有配置。
...
19. 已完成 source 准入：GitHub Actions source batch [`34002789451`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34002789451) 恢复 SDK 而未重建，以 `BUILD_SHARED=no` 构建锁定的 Buildroot LZ4 1.10.0 `lib/` 与 `programs/` 目录，并在 unsigned artifact [`9980027622`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34002789451/artifacts/9980027622) 中上传 `lz4_1.10.0-1_riscv64.ipk`。它只提供私有 `tdvp-lz4` command frontend，不提供 `liblz4` ABI、headers、pkg-config metadata 或普通 firmware command path。无重编 merge [`34003078163`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34003078163) 逐一比对全部 38 个 artifact manifest/IPK hash、重建 index，并在未编译的情况下通过 feed verification、runtime closure 与 445-object target coverage；SDK-base 与 source-build job 都是 skipped，且已上传 merged artifact [`9980152139`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34003078163/artifacts/9980152139)。CI 未执行 LZ4 或传入内容。它仍未签名，且仍需实机生命周期验证；
20. 已完成 source 准入：GitHub Actions source batch [`34004037276`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004037276) 恢复 SDK 而未重建，以 `-DENABLE_SHARED=OFF -DENABLE_STATIC=ON` 配置锁定的 LZO 2.10、链接 `liblzo2.a`，并在 unsigned artifact [`9980420311`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004037276/artifacts/9980420311) 中交叉构建私有 `lzop_1.04-1_riscv64.ipk`。其 command 是私有 `tdvp-lzop`，绝不成为 `liblzo2` ABI provider、header package、pkg-config provider 或普通 firmware command path。无重编 merge [`34004352420`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004352420) 逐一比对全部 39 个 artifact manifest/IPK hash、重建 index，并在未编译的情况下通过 feed verification、runtime closure 与 445-object target coverage；SDK-base 与 source-build job 都是 skipped，且已上传 merged artifact [`9980529319`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34004352420/artifacts/9980529319)。CI 未执行 LZOP 或传入内容。它仍未签名，且仍需实机生命周期验证；
21. 已完成 psmisc 23.7 source 准入与 merge：source batch [`34006256552`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552) 恢复而非重建 SDK；只有 archive 名和 SHA-256 与 `source.lock` 精确匹配时才临时提供一行 `psmisc.hash` override，普通 Buildroot 哈希验证保持开启，原清单在成功和失败时均按字节还原，并构建 `psmisc_23.7-1_riscv64.ipk`。它上传 GitHub SHA-256 为 `db9fcdbde1a26de09dcb5270f53213d9c5d514c1a50521243e7442eb7525f58a` 的 unsigned artifact [`9981057795`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552/artifacts/9981057795)。无重编 merge [`34006620790`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006620790) 逐一比对全部 40 个输入 artifact manifest/IPK hash、重建 index，并在未编译情况下通过 feed verification、runtime closure 和 445-object target coverage；SDK-base 与 package-build job 都是 skipped。它上传 merged unsigned artifact [`9981196020`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006620790/artifacts/9981196020)（197,245,213 bytes；GitHub SHA-256 `3abcbefac768ad21ebe49df0d54050b0d191e2891687e495dc0899a19d53e91d`）。该包只为 `fuser`、`killall`、`pslog`、`prtstat`、`pstree` 提供 private `tdvp-*` frontend，只消费 immutable `libncursesw`，排除无 RISC-V 支持的 `peekfd`；CI 未执行命令或检查 procfs。artifact 仍未签名，仍需实机生命周期验证；然后
22. 已完成 `net-tools 2.10` source 准入与 merge：source batch [`34007398511`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007398511) 恢复而非重建 SDK，采用与 Buildroot 2025.02.1 完全一致的源码版本/摘要，交叉构建 `net-tools_2.10-1_riscv64.ipk`，并通过 source、RISC-V ELF、dependency closure、feed 和 445-object target coverage gate。它上传 unsigned artifact [`9981425511`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007398511/artifacts/9981425511)（90,328,415 bytes；GitHub SHA-256 `9761e314772218eed4b4ee9c6fd14ef50fadfd158289b3a443fc35c80c8efc25`）；SDK-base 与 merge job 均 skipped。无重编 merge [`34007657490`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007657490) 逐一比对全部 41 个输入 artifact manifest/IPK hash、重建 index，并在未编译情况下通过 feed verification、runtime closure 和 445-object target coverage；SDK-base 与 package-build job 均 skipped。它上传 merged unsigned artifact [`9981521824`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34007657490/artifacts/9981521824)（197,486,158 bytes；GitHub SHA-256 `061756699b615cc167773297cec41b03562588eba3bd296aeefa3202f4f52ba2`）。IPK 仅提供经审核的私有 `tdvp-arp`、`tdvp-ifconfig`、`tdvp-ipmaddr`、`tdvp-iptunnel`、`tdvp-mii-tool`、`tdvp-nameif`、`tdvp-netstat`、`tdvp-plipconfig`、`tdvp-rarp`、`tdvp-route` 与 `tdvp-slattach` 前端。CI 只交叉构建并审计，既不调用这些工具，也不查询或改变网络状态。artifact 未签名，仍需发布授权与实机生命周期验证；然后
23. 为每个 release 生成来源证明/SBOM，并把来源、SDK 和测试结果与签名 release 对应；
24. 同时逐步引入经过审查的通用库，每次均保留共享运行时和实机测试门。
