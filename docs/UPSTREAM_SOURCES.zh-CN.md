...
10. source batch 已完成：GitHub Actions source batch [`33992855036`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33992855036) 已准入锁定独立 `jo` command，正待无重编 merge 合入。它保持私有、不引入 shared provider，CI 未执行或传入 JSON input；
11. 进行中：通过 GitHub Actions source batch 准入锁定 GNU `ed` line-editor command，再由无重编 merge 合入。host-lzip 只能解包 runner source，CI 不得执行编辑器或传入文件；
