...
7. 进行中：GitHub Actions source batch [`33989899026`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33989899026) 已准入锁定的 static-only `i2c-tools` 硬件检查 leaf；接下来只 hash-merge 已验证 artifact，不重编 source package。它已禁用 Python/py-smbus 与 shared `libi2c`，CI 未检查或变更任何硬件总线；
8. 为每个 release 生成来源证明/SBOM，并把来源、SDK 和测试结果与签名 release 对应；
9. 同时逐步引入经过审查的通用库，每次均保留共享运行时和实机测试门。
