...
9. 进行中：通过 GitHub Actions source batch 准入锁定 command-only `logrotate` leaf，再由无重编 merge 合入。该 package 必须禁用 SELinux/ACL，只复用 immutable target `libpopt` provider，排除 `/etc` 配置、timer 和 daemon payload，CI 不得运行任何日志轮转操作；
