...
21. 已完成 psmisc 23.7 source-build 准入：GitHub Actions source batch [`34006256552`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552) 恢复而非重建 SDK；只有 archive 名和 SHA-256 与 `source.lock` 精确匹配时才临时提供一行 `psmisc.hash` override，普通 Buildroot 哈希验证保持开启，原清单在成功和失败时均按字节还原。它从锁定 cache 构建 `psmisc_23.7-1_riscv64.ipk`，通过 source/ELF/closure/deny-overlay/feed/445-object target coverage gate，并上传 GitHub SHA-256 为 `db9fcdbde1a26de09dcb5270f53213d9c5d514c1a50521243e7442eb7525f58a` 的 unsigned artifact [`9981057795`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/34006256552/artifacts/9981057795)；SDK-base 与 merge job 都是 skipped。它只为 `fuser`、`killall`、`pslog`、`prtstat`、`pstree` 提供 private `tdvp-*` frontend，只消费 immutable `libncursesw`，排除无 RISC-V 支持的 `peekfd`。CI 未执行命令或检查 procfs。全 artifact 无重编 merge、签名、发布和实机生命周期验证仍待完成；然后
