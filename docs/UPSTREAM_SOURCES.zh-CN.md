...
21. 将 psmisc 23.7 锁定为上游版本增量的进程诊断候选：Buildroot 2025.02.1 的 `psmisc.mk` 仅保留为受审交叉构建逻辑，以一个受限的 `PSMISC_VERSION=23.7` Make variable 覆盖版本，并只预热官方 `psmisc-23.7.tar.xz`，其 SHA-256 为 `58c55d9c1402474065adae669511c191de374b0871eec781239ab400b907c327`。因基线 `psmisc.hash` 不含该 release，只能在 archive 名和 SHA-256 与 `source.lock` 精确匹配后临时提供一行 override；正常 Buildroot 哈希验证保持开启，无论成功或失败均按字节还原原清单。它只可发布 private `fuser`、`killall`、`pslog`、`prtstat`、`pstree` 和不同的 `tdvp-*` frontend；只复用精确 immutable `libncursesw`，排除尚未支持 RISC-V 的 `peekfd`。其 batch 必须恢复而非重建 SDK，只编译/审计、绝不检查 procfs 或执行命令，通过 source、ELF、overlay 和 closure gate 后才可进入全量 artifact 无重编 merge；然后
