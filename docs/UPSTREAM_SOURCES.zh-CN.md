...
6. 进行中：GitHub Actions source batch [`33988534267`](https://github.com/vicliu624/embedded-opkg-feed/actions/runs/33988534267) 已准入单独锁定、命名空间隔离的 `gdbserver` 与 `ethtool` debug/network leaf；接下来只 hash-merge 已验证 artifact，不重编 source package。其 recipe 未改变 SDK host state、firmware command path 或网络接口；
