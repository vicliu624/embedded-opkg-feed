...
22. `net-tools 2.10` source-lock 候选使用与 Buildroot 2025.02.1 完全一致的源码版本和摘要。它仅提供经审核的私有 `tdvp-arp`、`tdvp-ifconfig`、`tdvp-ipmaddr`、`tdvp-iptunnel`、`tdvp-mii-tool`、`tdvp-nameif`、`tdvp-netstat`、`tdvp-plipconfig`、`tdvp-rarp`、`tdvp-route` 与 `tdvp-slattach` 前端。CI 只交叉构建并审计载荷，既不调用这些工具，也不查询或改变网络状态。只有源码、ELF 和闭包校验全部通过，未签名工件才可进入全工件合并；然后
