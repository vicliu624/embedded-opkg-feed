...
17. 进行中：把锁定的 GNU `findutils` 4.10.0 command leaf 加入 GitHub Actions dispatch matrix。它只会封装私有 `find`/`xargs` ELF 与 TDVP 前缀 frontend，不封装 `locate`、`updatedb` 或 shared provider；CI 只构建/审计，绝不调用任一 command、传入 filesystem path 或启动子命令。source batch 与 no-recompile hash merge 均通过前仍未准入；
