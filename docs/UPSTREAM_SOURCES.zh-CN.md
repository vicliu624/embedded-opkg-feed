...
8. 进行中：通过 GitHub Actions source batch 准入锁定 no-shared-library `inotify-tools` leaf，再由无重编 merge 合入。该 package 必须禁用 shared `libinotifytools`，只公开私有 `inotifywait`/`inotifywatch` command ELF，CI 不得启动 watcher 或观察 filesystem path；
9. 为每个 release 生成来源证明/SBOM，并把来源、SDK 和测试结果与签名 release 对应；
10. 同时逐步引入经过审查的通用库，每次均保留共享运行时和实机测试门。
