...
env -i HOME="${HOME:-/tmp}" USER="${USER:-tdvp}" LOGNAME="${LOGNAME:-tdvp}" PATH="$sdk_root/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" BR2_DL_DIR="$download_dir" BR2_PRIMARY_SITE="file://$download_dir" make -C "$build_output" tdvp-audacious-plugins-source tdvp-audacious-source alsa-lib-source pulseaudio-source ffmpeg-source libglib2-source libgtk3-source zlib-source
