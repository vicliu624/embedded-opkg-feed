...
[[ -d "$sdk_root" && -d "$support_dir" && -d "${TDVP_FEED_STAGING_ROOT:-}" ]] || { echo 'audacious-plugins needs the core staging root and matching Buildroot SDK' >&2; exit 66; }
...
cleanup() {
  ...
  if [[ "$config_saved" -eq 1 ]]; then
    ...
    # Restore generated Buildroot configuration as well as the tracked
    # .config.  This keeps a feed-only plugin build from contaminating a
    # later firmware image build in the same output directory.
...
env -i HOME="${HOME:-/tmp}" USER="${USER:-tdvp}" LOGNAME="${LOGNAME:-tdvp}" PATH="$sdk_root/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" make -C "$build_output" tdvp-audacious-plugins-dirclean
env -i HOME="${HOME:-/tmp}" USER="${USER:-tdvp}" LOGNAME="${LOGNAME:-tdvp}" PATH="$sdk_root/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" make -C "$build_output" TARGET_DIR="$install_root" tdvp-audacious-plugins-install-target
