#!/usr/bin/env bash
# Build the locked GNU bc source with Buildroot and expose it only under a
# TDVP-prefixed frontend; this never overwrites a firmware BusyBox command.
set -Eeuo pipefail
IFS=$'\n\t'
...
TDVP_COMMAND_FRONTEND_NAMES='bc=tdvp-bc' \
  tdvp_buildroot_command_package "$package_dir" "$4" "${TDVP_DEVEL_BUILDROOT_OUTPUT:-}" \
    BR2_PACKAGE_BC bc 'BC_VERSION = 1.07.1' 'bc'
