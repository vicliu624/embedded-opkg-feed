#!/usr/bin/env bash
# Build OpenSSL's public libcrypto SONAME as a reusable feed provider.
set -Eeuo pipefail
IFS=$'\n\t'
...
[[ "$2" == tdvp-k230-r1 ]] || {
  echo "libcrypto-3 does not support platform: $2" >&2
  exit 65
}

package_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
sdk_root=$4
configured_output=$(printenv TDVP_LIBOPENSSL_BUILDROOT_OUTPUT || true)
...
tdvp_build_archive_library "$package_dir" "$sdk_root" "$configured_output" \
  BR2_PACKAGE_LIBOPENSSL libopenssl 'libcrypto.so*' 'LIBOPENSSL_VERSION = 3.4.1'
