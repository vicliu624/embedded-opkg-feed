#!/usr/bin/env bash
# Build OpenSSL's public libssl SONAME as a reusable feed provider.
set -Eeuo pipefail
IFS=$'\n\t'
...
[[ "$2" == tdvp-k230-r1 ]] || {
  echo "libssl-3 does not support platform: $2" >&2
  exit 65
}
...
tdvp_build_archive_library "$package_dir" "$sdk_root" "$configured_output" \
  BR2_PACKAGE_LIBOPENSSL libopenssl 'libssl.so*' 'LIBOPENSSL_VERSION = 3.4.1'
