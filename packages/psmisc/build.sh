...
# cache and temporarily replaces the package hash manifest with the sole row
# that source.lock approves. The original Buildroot manifest is restored after
# every transaction. Five ordinary process commands remain under a private
# libexec tree.
...
TDVP_COMMAND_BUILDROOT_HASH_OVERRIDE_FILE='buildroot.hash.override' \
