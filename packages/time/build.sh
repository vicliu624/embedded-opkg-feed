#!/usr/bin/env bash
# Build a private GNU time command from the locked GNU release.
set -Eeuo pipefail
IFS=$'\n\t'
...
# CI builds and audits the ELF only. It never invokes time or lets it execute
# a child command to measure process resources.
TDVP_COMMAND_FRONTEND_NAMES='time=tdvp-time' \
  tdvp_buildroot_command_package "$package_dir" "$4" "${TDVP_SYSTEM_BUILDROOT_OUTPUT:-}" \
    BR2_PACKAGE_TIME time 'TIME_VERSION = 1.9' 'time'
