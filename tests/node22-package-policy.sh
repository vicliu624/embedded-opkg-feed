...
grep -Fq 'TOOLSET_SHARED_INTERMEDIATE_DIR' "$node_build"
grep -Fq 'shared_dir = "gen" if self.toolset == "target" else f"gen.{self.toolset}"' "$node_build"
...
grep -Fq 'command -v qemu-riscv64-static' "$node_build"
grep -Fq 'tar make patch cmake ninja' "$node_build"
