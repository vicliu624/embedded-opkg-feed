...
bash -n "$script"
bash -n "$cohort"
for entry in libevent tmux dialog ncdu pv netcat iperf3 lsof openssh-client libcap-2 sqlite3 bc coreutils mtools dosfstools util-linux exfatprogs memtester tree less; do grep -Fq "$entry" "$cohort"; done
