...
bash -n "$script"
bash -n "$cohort"
for entry in libevent tmux dialog ncdu pv netcat iperf3 lsof openssh-client sqlite3 bc coreutils mtools dosfstools util-linux exfatprogs memtester tree less gdbserver ethtool i2c-tools inotify-tools logrotate jo ed cpio time cpulimit bwm-ng rhash; do grep -Fq "$entry" "$cohort"; done
