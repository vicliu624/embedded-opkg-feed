...
bash -n "$script"
bash -n "$cohort"
for entry in libevent tmux dialog ncdu pv netcat iperf3 lsof sqlite3 bc coreutils mtools dosfstools util-linux exfatprogs memtester tree; do grep -Fq "$entry" "$cohort"; done
