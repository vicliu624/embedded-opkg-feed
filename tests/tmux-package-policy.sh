...
grep -Fq 'Its cohort builds locked-source libevent' "$repo_root/packages/tmux/source.lock"
grep -Fq 'BR2_USE_MMU=y BR2_USE_WCHAR=y BR2_ENABLE_LOCALE=y BR2_PACKAGE_NCURSES=y' "$repo_root/packages/tmux/build.sh"
grep -Fq 'TMUX_CONF_OPTS=--disable-systemd --disable-utf8proc' "$repo_root/packages/tmux/build.sh"
grep -Fq 'TMUX_DEPENDENCIES=libevent ncurses host-pkgconf' "$repo_root/packages/tmux/build.sh"
grep -Fq 'never mutate or try to disable the completed SDK' "$repo_root/packages/tmux/build.sh"
