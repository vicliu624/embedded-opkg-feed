...
grep -Fq 'libevent may retain its target-attested OpenSSL closure' "$repo_root/packages/tmux/source.lock"
grep -Fq 'BR2_USE_MMU=y BR2_USE_WCHAR=y BR2_ENABLE_LOCALE=y BR2_PACKAGE_NCURSES=y' "$repo_root/packages/tmux/build.sh"
grep -Fq "TDVP_COMMAND_BUILDROOT_DISABLE_SYMBOLS='BR2_PACKAGE_SYSTEMD BR2_PACKAGE_UTF8PROC'" "$repo_root/packages/tmux/build.sh"
grep -Fq "must not attempt to" "$repo_root/packages/tmux/build.sh"
