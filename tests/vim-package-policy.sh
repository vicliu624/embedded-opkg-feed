#!/usr/bin/env bash
# Keep terminal Vim, its r8 package split, and pure-plugin boundary explicit.
set -Eeuo pipefail
IFS=$'\n\t'
...
for plugin in repeat surround commentary sleuth; do
  ...
  expect_line "^PACKAGE_RELEASES='r8'$" "$env_file"
...
echo 'Vim r8 package, touch/keyboard defaults, and pure-plugin policy: PASS'
